...
const TRACK_TITLE_PROMPT = `너는 음악 메타데이터 교정 도우미야. Spotify 검색 결과로 받은 곡 제목 중,
한국어 원제가 있는데 영어로 번역되어 표시된 제목만 한국어 원제로 고쳐야 해.

반드시 아래 JSON 형식으로만 응답해. 다른 설명, 코드블록 표시 없이 순수 JSON만 출력해:
{
  "tracks": [
    { "id": "입력받은 id", "title": "화면에 표시할 제목" }
  ]
}

규칙:
- 원래 제목이 영어인 곡은 절대 번역하지 말고 입력 제목 그대로 둬.
- 한국어 원제가 널리 알려진 곡이 영어 번역 제목으로 들어온 경우에만 한국어 원제로 바꿔.
- 확실하지 않으면 입력 제목 그대로 둬.
- 괄호 안 버전 정보, 리마스터, feat. 표기는 원제 판단에 꼭 필요한 경우만 유지해.
- 입력받은 모든 id를 그대로 포함하고, id를 바꾸거나 새 곡을 추가하지 마.`;
...
/**
 * Spotify가 한국 곡 제목을 영어 번역명으로 돌려준 경우, 표시 제목만 한국어 원제로 보정한다.
 * 원래 영어 제목인 곡은 그대로 유지한다.
 * @param {Array<{id:string,title:string,artist:string,album:string}>} tracks
 * @returns {Promise<Array>}
 */
async function restoreKoreanTrackTitles(tracks) {
  ...
  const payload = tracks.slice(0, 10).map(track => ({
    id: String(track.id || ''),
  ...
  if (payload.length === 0) return tracks;

  const res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      system_instruction: { parts: [{ text: TRACK_TITLE_PROMPT }] },
  ...
  const data = await res.json();
  const rawText = data?.candidates?.[0]?.content?.parts?.[0]?.text || '';
  const restoredTitleMap = parseGeminiTrackTitles(rawText, tracks);

  return tracks.map(track => ({
    ...track,
    title: restoredTitleMap.get(track.id) || track.title,
  }));
}
...
function parseGeminiTrackTitles(rawText, originalTracks) {
  const cleaned = rawText.trim().replace(/^```json\s*|^```\s*|```$/g, '').trim();
...
module.exports = { interpretSearchQuery, suggestArtistAlbumNames, restoreKoreanTrackTitles };
