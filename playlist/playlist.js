(() => {
  ...
  function normalizeImportedTrack(track, fallbackCoverUrl = '') {
    return {
  ...
  function normalizePlaylistForPointer(playlist) {
    const tracks = Array.isArray(playlist?.tracks) ? playlist.tracks.map(track => normalizeImportedTrack(track, playlist.coverUrl)).filter(track => track.id && track.title && track.artist) : [];
    return {
  ...
  async function setPointerPlaylist(playlist, x = pointerPlaylistX, y = pointerPlaylistY) {
    ...
    try {
      const res = await fetch(`/api/playlists/${encodeURIComponent(playlist.id)}`, { credentials: 'same-origin' });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || '플레이리스트를 불러오지 못했어요.');
      pointerPlaylist = normalizePlaylistForPointer(data.playlist);
    } catch (_) {
  ...
  window.addEventListener('keydown', e => {
    if (e.key === 'Escape' && pointerPlaylist) clearPointerPlaylist();
  });
  window.addEventListener('pointermove', e => positionPointerPlaylist(e.clientX, e.clientY), { passive: true });
